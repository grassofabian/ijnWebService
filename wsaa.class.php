...
class WSAA {
   ...
   * Constructor
   */
  public function __construct($pt, $P_o_T)
  {  	
    $this->path = './';
    $this->service = 'wsfe';  
    ...
    $this->cert=$pt.$contenido["Certificados"]["CRT"];
    $this->privatekey = $pt.$contenido["Certificados"]["KEY"];
  ...
  private function create_TRA()
  {
    ...
    */
    ...
    if($TA_file) {
    $TA_xml = '';
    for($i=0; $i < sizeof($TA_file); $i++) $TA_xml.=$TA_file[$i]; $this->TA = $this->xml2Array($TA_xml);
